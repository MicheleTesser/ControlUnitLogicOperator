...
int8_t core_1_feature_init(Core1Feature_h* const restrict self)
{
    ...
    if(core_1_imu_init(&p_self->m_core_1_imu, &p_self->m_log)<0) return -5;
    if(suspensions_init(&p_self->m_suspensions, &p_self->m_log)<0) return -6;
    if(system_settings_init(&p_self->m_setting_update)<0)return -7;
