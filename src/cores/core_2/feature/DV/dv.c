...
static void _dv_set_status(struct Dv_t *const restrict self,
                           const enum AS_STATUS m_status) {
...
static int8_t _dv_update_led(struct Dv_t *const restrict self) {
  ...
  ACTION_ON_FREQUENCY(self->m_emergency_sound_last_time_on,
                      get_tick_from_millis(8000)) {
...
static int8_t _dv_update_status(struct Dv_t *const restrict self) {
  ...
  if (as_node_get_status(&self->m_as_node) &&
      ebs_asb_consistency_check(&self->m_dv_ebs)) {
    switch (giei_status) {
    case SYSTEM_OFF:
  ...
  } else if (self->m_dv_mission_status == MISSION_FINISHED && !current_speed &&
             !_sdc_closed(self)) {
...
int8_t dv_class_init(Dv_h *const restrict self,
  ...
  if (hardware_init_gpio_pwm(&p_self->m_gpio_ass_light_yellow,
                             PWM_GPIO_ASSI_LIGHT_YELLOW) < 0) {
    return -5;
  }

  if (hardware_init_gpio_pwm(&p_self->m_gpio_ass_light_blue,
  ...
  ACTION_ON_CAN_NODE(CAN_GENERAL, can_node) {
    p_self->p_send_car_m_car_status_mailbox = hardware_get_mailbox_single_mex(
        can_node, SEND_MAILBOX, CAN_ID_CARMISSIONSTATUS,
  ...
  if (shared_message_reader_init(&p_self->m_recv_mission_status,
                                 SHARED_MEX_DV_MISSION)) {
    hardware_free_mailbox_can(&p_self->p_send_car_m_car_status_mailbox);
    return -11;
  }

  ACTION_ON_CAN_NODE(CAN_GENERAL, can_node) {
    p_self->p_mailbox_send_mission_can_2 = hardware_get_mailbox_single_mex(
        can_node, RECV_MAILBOX, CAN_ID_CARMISSIONSTATUS,
  ...
  if (shared_message_reader_init(&p_self->m_recv_embedded_alive,
...
#define ERR_TRACE()                                                            \
  ...
  case CAR_MISSIONS_DV_INSPECTION:
    ...
    if (imu_update(&p_self->m_imu) < 0)
      ERR_TRACE();
    if (ebs_update(&p_self->m_dv_ebs) < 0)
      ERR_TRACE();
    ...
    if ((timer_time_now() - p_self->m_embeed_last_alive) >
        get_tick_from_millis(500)) {
      p_self->m_dv_mission_status = MISSION_NOT_RUNNING;
    ...
    if (shared_message_read_unpack_can3(&p_self->m_recv_mission_status, &o3) >
        0) {
      p_self->m_dv_mission_status =
          o3.can_0x07e_DV_EMBEDDED_Status.Embedded_status;
    }

    if (p_self->m_status == AS_DRIVING) {
      // TODO: add check continuous monitoring
      dv_stw_alg_compute(&input_stw_alg,
                         &output_stw_alg); // TODO: not yet implemented
    }

    if (_dv_update_status(p_self) < 0)
      ERR_TRACE();
  ...
  o2.can_0x071_CarMissionStatus.Mission =
      car_mission_reader_get_current_mission(p_self->p_mission_reader);
  ...
  hardware_mailbox_send(p_self->p_send_car_m_car_status_mailbox,
