 ...
 *      1  => other_key > tree_key
 */
typedef int8_t key_compare(const void* const restrict tree_key, 
        const void* const restrict other_key);
typedef int8_t key_update(void* const restrict tree_key);

struct bst* bst_new(key_compare cmp_fun_1, key_compare cmp_fun_2,
...
int8_t bst_update_existing(struct bst* self, const void* const restrict key,
...
int8_t bst_free(struct bst* const restrict self);
