...
static int8_t cmp_key_info(const void* const restrict tree_key, 
        const void* const restrict other_key)
{
    const struct MexInfo* tree_info = tree_key;
    const struct MexInfo* other_info = other_key;
    return memcmp(&other_info->can_id, &tree_info->can_id, sizeof(other_info->can_id));
}

static int8_t cmp_key_freq(const void* const restrict tree_key,
        const void* const restrict other_key)
{
    const struct MexInfo* tree_info = tree_key;
    const struct MexInfo* other_info = other_key;

    if (other_info->deadline < tree_info->deadline)
    {
        return -1;
    }
    else if (other_info->deadline > tree_info->deadline)
    {
        return 1;
    }
    else
...
{
    struct MexInfo* tree_info = tree_key;
    tree_info->deadline = timer_time_now() + tree_info->freq;
...
int8_t can_freq_class_init(const uint16_t amount_of_ids)
{
    ...
    FREQ_TREE = bst_new(cmp_key_info, cmp_key_freq,
...
int8_t can_freq_update_freq(const uint16_t can_id)
{
    ...
    const struct MexInfo* p_info = bst_search(FREQ_TREE, &info_s, ID_FILTER);
    info_s.deadline = timer_time_now() + p_info->freq;
    return 0;
}
